...
const CheckoutCallback = () => {
  ...
  // ✅ Guard — prevents double-confirm on re-render or StrictMode double-invoke
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      // ✅ Bail out immediately if already confirmed
      ...
      try {
        ...
        if (!statusData.success) {
          setStatus('failed');
          setMessage(statusData.message || 'Could not verify payment.');
          return;
        }
        ...
        if (state === 'COMPLETED') {
          // ✅ Mark confirmed BEFORE calling API — prevents race condition
          confirmedRef.current = true;

          const saved = JSON.parse(sessionStorage.getItem('pendingOrder') || '{}');

          // Guard — if sessionStorage was already cleared (double load), show success
          if (!saved.guestEmail && !saved.email) {
            setStatus('success');
          ...
          if (confirmData.success) {
            ...
            setStatus('success');
            setMessage(`Order confirmed! A confirmation email has been sent to ${saved.guestEmail || saved.email || 'your email'}.`);
          } else {
            confirmedRef.current = false; // allow retry on genuine failure
            setStatus('failed');
            setMessage(confirmData.message || 'Order placement failed. Please contact support.');
          }
  ...
  return (
    <div style={{ minHeight:'70vh', display:'flex', alignItems:'center', justifyContent:'center', padding:'2rem' }}>
      <div style={{ textAlign:'center', maxWidth:'480px' }}>
        ...
        {status === 'checking' && <div style={{ fontSize:'3rem', marginBottom:'1rem' }}>⏳</div>}
        {status === 'success' && (
          <div style={{ width:72, height:72, borderRadius:'50%', background:'#e8f5e9', color:'#2e7d32', fontSize:'2rem', fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 1.5rem' }}>✓</div>
        )}
        {(status === 'failed' || status === 'pending') && (
          <div style={{ fontSize:'2.5rem', marginBottom:'1rem' }}>{status === 'failed' ? '✗' : '⏳'}</div>
        )}
        ...
        {status === 'failed' && (
          <div style={{ display:'flex', gap:'1rem', justifyContent:'center', flexWrap:'wrap' }}>
            <button className="btn-primary" onClick={() => navigate('/checkout')}
              style={{ padding:'0.85rem 2rem', fontSize:'1rem' }}>Try Again</button>
            <button className="btn-secondary" onClick={() => navigate('/')}
              style={{ padding:'0.85rem 2rem', fontSize:'1rem' }}>Return Home</button>
          </div>
        )}

        {status === 'pending' && (
          <button className="btn-primary" onClick={() => window.location.reload()}
            style={{ padding:'0.85rem 2.5rem', fontSize:'1rem' }}>Refresh Status</button>
        )}
